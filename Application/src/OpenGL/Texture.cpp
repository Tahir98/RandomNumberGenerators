...
namespace OpenGL {
	Texture::Texture(const char* path) : filePath(path), width(0), height(0), bpp(0), id(0) {
		...
		if (image == nullptr) {
			APP_LOG_ERROR("Image not found!, path:{0}", path);
		}
		else {
			glGenTextures(1, &id);
			...
			APP_LOG_INFO("Texture created, id:{0}, width:{1}, Height:{2}, BPP:{3}", id, width, height, bpp);
	...
	Texture::~Texture() {
		APP_LOG_INFO("Texture deleted id:{0}", id);
