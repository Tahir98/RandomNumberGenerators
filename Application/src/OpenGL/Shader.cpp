...
namespace OpenGL {
	...
	Shader::~Shader() {
		APP_LOG_INFO("Shader(program) deleted, id:{0}", id);
		glDeleteProgram(id);
	}
	...
	void Shader::compileShader() {
		...
		if (!stream.is_open()) {
			APP_LOG_ERROR("File not found: {0}", filePath);
		}
		...
		if (!success) {
			...
			APP_LOG_ERROR("FAILED TO COMPILE VERTEX SHADER: {0}", message);
		...
		if (!success) {
			...
			APP_LOG_ERROR("FAILED TO COMPILE FRAGMENT SHADER: {0}", message);
		...
		if (!success) {
			...
			APP_LOG_ERROR("FAILED TO LINK SHADER(PROGRAM): {0}", message);
			free(message);
		}
		...
		if (!success) {
			...
			APP_LOG_ERROR("FAILED TO VALIDATE SHADER(PROGRAM): {0}", message);
			free(message);
			exit(-1);
		}
		...
		APP_LOG_INFO("Shader(program) created, id:{0}", id);
	}
	...
	void Shader::setUniform1i(const char* name, const int& value) {
		...
		if (location != -1)
			...
			APP_LOG_ERROR("{0}: UNIFORM LOCATION NOT FOUND", name);
	}
