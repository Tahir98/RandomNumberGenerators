...
public:
	SpherecalSkybox(const char* texturePath) {
		...
		vb = new OpenGL::VertexBuffer((float*)vertices.data(), sizeof(Vec3) * vertices.size(), GL_STATIC_DRAW);
		ib = new OpenGL::IndexBuffer(indices.data(), sizeof(unsigned int) * indices.size(), GL_STATIC_DRAW);
		...
		if (image == nullptr) {
			APP_LOG_ERROR("Texture not found: {0}", texturePath);
		}
		else {
			...
			APP_LOG_INFO("Texture loaded successfuly, {0}, width: {1}, height: {2}, bpp: {3}", texturePath, width, height, bpp);
		...
		shader = new OpenGL::Shader("Shaders/skybox.shader");
