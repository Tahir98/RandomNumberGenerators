...
public:
	...
	void setParemeters(const uint32_t j, const uint32_t k, const uint32_t m) {
		if (j >= k) {
			RAND_LOG_ERROR("Error, Lagged Finonacci Generator: k must be bigger than j [{0},{1}]", j, k);
			return;
		}
		else if (j <= 0) {
			RAND_LOG_ERROR("Error, Lagged Finonacci Generator: j must be bigger than 0, j = {0}]", j);
			return;
		}
		else if (m < 2) {
			RAND_LOG_ERROR("Error, Lagged Finonacci Generator: m must be bigger than 0, m = {0}]", m);
			return;
		}
