...
public:
	...
	void setParemeters(const uint32_t j, const uint32_t k, const uint64_t m) {
		if (j >= k) {
			RAND_LOG_ERROR("Error, Lagged Finonacci Generator64: k must be bigger than j [{0},{1}]", j, k);
			return;
		}
		else if (j <= 0) {
			RAND_LOG_ERROR("Error, Lagged Finonacci Generator64: j must be bigger than 0, j = {0}]", j);
			return;
		}
		else if (m < 2) {
			RAND_LOG_ERROR("Error, Lagged Finonacci Generator64: m must be bigger than 0, m = {0}]", m);
			return;
		}
	...
	uint64_t nextUInt64(const uint64_t min, const uint64_t max) {
		if (min >= max) {
			RAND_LOG_ERROR("Error, Lagged Finonacci Generator64: max must be bigger than min [{0},{1}]", min, max);
			return 0;
		}
