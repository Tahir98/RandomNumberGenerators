...
public:
	...
	void setSize(uint32_t size) {
		if (size == 0) {
			RAND_LOG_ERROR("Multply With Carry: state vector size must be at least 1");
			return;
		}
		this->size = size;
	...
	void setParameters(const uint32_t a, const uint32_t power, const uint32_t c) {
		if (a == 0 || power == 0) {
			RAND_LOG_ERROR("a and b must be bigger than 1");
			return;
		}
		else if (power > 64) {
			RAND_LOG_ERROR("b must be smaller or equal than 64");
		}
	...
	uint32_t nextUInt(const uint32_t min, const uint32_t max) override {
		if (min >= max) {
			RAND_LOG_ERROR("Multiply With Carry: Max must be bigger than min");
			return 0;
		}
