...
public:
	...
	void setShiftingParemeters(const uint32_t a, const uint32_t b, const uint32_t c) {
		if (a == 0 || a > 31)
			RAND_LOG_ERROR("Error, Xorshift, a must be in [1,31], given: {0}", a);
		else
			this->a = a;

		if (b == 0 || b > 31)
			RAND_LOG_ERROR("Error, Xorshift, b must be in [1,31], given: {0}", b);
		else
			this->b = b;

		if (c == 0 || c > 31)
			RAND_LOG_ERROR("Error, Xorshift, c must be in [1,31], given: {0}", a);
		else
			this->c = c;
	}
	...
	uint64_t nextUInt64(const uint64_t min, const uint64_t max) {
		if (min >= max) {
			RAND_LOG_ERROR("Error, Xorshift: min is bigger or equal than max");
			return 0;
		}
